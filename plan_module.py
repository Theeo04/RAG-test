    ...
    for r in meta:
        if r["component"] == component and r["type"] == "values":
            ...
            out.append({
                ...
                "candidate_keys": keys,
                "component": component,  # annotate
            })
    out.sort(key=lambda v: (0 if (v.get("profile") or "").lower() == "common" else 1,
                            v.get("profile") or "zzz", v["path"]))
    return out

def _all_manifest_files_for_component(component: str,
                                      meta: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
    Returnează lista unică de manifeste pentru componentă, cu kinds + images detectate.
    """
    seen = set()
    out = []
    for r in meta:
        if r["component"] == component and r["type"] == "manifest":
            if r["path"] in seen:
                continue
            seen.add(r["path"])
            out.append({
...
def do_plan(root: str,
    ...
    # select components: by threshold or top-N
    if min_score is not None:
        selected_components = [e["component"] for e in comp_rank if e["score"] >= float(min_score)]
        if not selected_components:
            selected_components = [comp_rank[0]["component"]]
    else:
        selected_components = [e["component"] for e in comp_rank[:max(1, int(top_components))]]

    # collect values/manifests for selected components
    values_files = []
    manifest_files = []
    for comp in selected_components:
        vfs = _all_values_files_for_component(comp, meta, extract_keys_fn)
        for vf in vfs:
            ks = vf.get("candidate_keys") or []
            vf["keys_count"] = len(ks)
            vf["keys_sample"] = ks[:20]
        values_files.extend(vfs)
        manifest_files.extend(_all_manifest_files_for_component(comp, meta))

    target_comp = selected_components[0]
    ...
    return {
        ...
        "targets": [{"component": c} for c in selected_components],
